...
def _make_happ_tv_button(email_or_key: str) -> InlineKeyboardButton:
    return InlineKeyboardButton(text="📺 Happ TV", callback_data=f"happ_tv|{email_or_key}")


def _make_profile_happ_tv_button() -> InlineKeyboardButton:
    return InlineKeyboardButton(text="📺 Happ TV", callback_data="happ_tv_profile")
...
async def start_happ_tv_from_profile(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    if not keys:
        ...
        await edit_or_send_message(
            target_message=callback.message,
            text="У вас нет активных подписок для подключения Happ TV.",
            reply_markup=kb.as_markup(),
        )
        return
    ...
    if len(keys) == 1:
        ...
        from .texts import HAPP_TV_CODE_REQUEST
        kb = InlineKeyboardBuilder()
        kb.row(InlineKeyboardButton(text=BACK, callback_data="happ_tv_profile_back"))
    ...
    await edit_or_send_message(
        target_message=callback.message,
        text="Выберите подписку для подключения Happ TV:",
        reply_markup=kb.as_markup(),
    )


@router.callback_query(F.data == "happ_tv_profile_back")
async def happ_tv_profile_back(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    try:
        await state.clear()
    except Exception:
        pass
    ...
    await process_callback_view_profile(callback, session, admin=False)
